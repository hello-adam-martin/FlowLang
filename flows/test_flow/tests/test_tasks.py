...
@pytest.mark.asyncio
async def test_process_action(registry):
    """
    Test ProcessAction task
    ...
    """
    # Get the task
    task = registry.get_task('ProcessAction')
    
    # Test inputs
    validated_data = 'test_id_123'
    
    # Currently expecting NotImplementedTaskError
    with pytest.raises(NotImplementedTaskError):
        await task(validated_data=validated_data)
    
    # TODO: After implementing, replace above with actual assertions:
    # result = await task(validated_data=validated_data)
...
@pytest.mark.skip(reason="Task not yet implemented")
@pytest.mark.asyncio
async def test_validate_input(registry):
    """
    Test ValidateInput task
    ...
    """
    # Get the task
    task = registry.get_task('ValidateInput')
    ...
    # Currently expecting NotImplementedTaskError
    with pytest.raises(NotImplementedTaskError):
        await task(action=action, user_id=user_id)
    
    # TODO: After implementing, replace above with actual assertions:
    # result = await task(action=action, user_id=user_id)
    # assert isinstance(result, dict)
    # assert "expected_key" in result
    # assert result["expected_key"] == expected_value
