...
@pytest.mark.asyncio
async def test_format_output(registry):
    """
    Test FormatOutput task
    ...
    """
    # Get the task
    task = registry.get_task('FormatOutput')
...
@pytest.mark.skip(reason="Task not yet implemented")
@pytest.mark.asyncio
async def test_process_message(registry):
    """
    Test ProcessMessage task
    ...
    """
    # Get the task
    task = registry.get_task('ProcessMessage')
    ...
    # Currently expecting NotImplementedTaskError
    with pytest.raises(NotImplementedTaskError):
        await task(text=text)
    
    # TODO: After implementing, replace above with actual assertions:
    # result = await task(text=text)
    # assert isinstance(result, dict)
    # assert "expected_key" in result
    # assert result["expected_key"] == expected_value
